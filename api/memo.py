...
@router.post("/update/{memo_id}", response_model = APIResponse[int])
...
@router.get("/all",response_model = APIResponse[List[MemoListSchema]])
def read_all_memos(
...
@router.get("/scrap", response_model=APIResponse[List[MemoListSchema]])
...
@router.get("/", response_model=APIResponse[List[MemoListSchema]])
def get_all_memos(
    current_user=Depends(get_current_user),
    db: Session = Depends(get_db),
):
...
@router.post("/unscrap/{memo_id}", response_model=APIResponse[bool])
def unscrap_memo_route(
    memo_id: int,
    current_user=Depends(get_current_user),
    db: Session = Depends(get_db),
):
    memo = unscrap_memo(db, current_user.user_id, memo_id)
    return APIResponse(success=True, data=memo)
...
@router.get("/user/{user_id}", response_model=APIResponse[List[MemoListSchema]])
def get_user_memos(
    user_id: int,
    db: Session = Depends(get_db),
):
